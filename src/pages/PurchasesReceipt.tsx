...
export default function PurchasesReceipt() {
  ...
  /** Normaliza um item vindo do parser para todos os shapes possíveis */
  function normalizeItem(it: any) {
    const name = String(it?.nome || "").trim() || "Item";
    const unit = String(it?.unidade || "un").toLowerCase();
    const isPeso = /^(kg|g|l|ml)$/.test(unit);

    const quantidade = Number(it?.quantidade) > 0 ? Number(it.quantidade) : 1;
    const peso = Number(it?.peso) > 0 ? Number(it.peso) : undefined;

    const totalParser =
      typeof it?.total === "number" && it.total > 0 ? Number(it.total) : undefined;

    // no parser: preco = unitário (itens de unidade) OU total (itens de peso)
    const precoParser = Number(it?.preco) || 0;
    ...
    if (isPeso) {
      qty = 1;
      // prioridade: total informado > precoParser (total) > precoParser*peso
      if (totalParser && totalParser > 0) total = totalParser;
      else if (precoParser > 0) total = precoParser;
      else if (peso && precoParser > 0) total = +(precoParser * peso).toFixed(2);

      const denom = peso && peso > 0 ? peso : 1;
      unitPrice = denom > 0 ? +(total / denom).toFixed(2) : 0;
    } else {
      qty = Math.max(1, quantidade);
      unitPrice = precoParser > 0 ? precoParser : totalParser ? +(totalParser / qty).toFixed(2) : 0;
      total = totalParser && totalParser > 0 ? totalParser : +(unitPrice * qty).toFixed(2);
    }

    return {
      // chaves usadas no detalhe (inclui aliases para garantir)
      name,
      title: name,
      nome: name,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // categoriza + enriquece imagem + normaliza campos
      const base = ((parsed as any)?.itens || []).map((it: any) => ({
        ...it,
        categoria: it?.categoria || categorize(it?.nome || ""),
      }));

      const itens = await Promise.all(
        base.map(async (it: any) => {
      ...
      // total da compra (alguns contextos usam isso)
      const purchaseTotal = itens.reduce((s: number, it: any) => s + (Number(it.total) || 0), 0);

      await createFromReceipt({
        name: listName || "Compra (NFC-e)",
        market: market || "—",
        date: dateObj,
        total: purchaseTotal, // ajuda a lista a não ficar R$ 0,00
        itens,                // mantém o campo itens
        items: itens,         // …e também em 'items' caso o contexto espere essa chave
        source: "receipt",
      });
